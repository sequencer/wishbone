...
case class WishboneSlaveParameters(
...
case class WishboneSlavePortParameters(slaves: Seq[WishboneSlaveParameters], beatBytes: Int) {
...
case class WishboneMasterParameters(name: String, nodePath: Seq[BaseNode] = Seq(), userBits: Seq[UserBits] = Nil) {
...
case class WishboneBundleParameters(
  addrBits:       Int,
  dataBits:       Int,
  selectBits:     Int,
  dataTagBits:    Int = 0,
...
object WishboneBundleParameters {
  val emptyBundleParams: WishboneBundleParameters = WishboneBundleParameters(addrBits = 1, dataBits = 8, selectBits = 1)

  def union(x: Seq[WishboneBundleParameters]): WishboneBundleParameters =
...
case class WishboneEdgeParameters(
  master:     WishboneMasterPortParameters,
