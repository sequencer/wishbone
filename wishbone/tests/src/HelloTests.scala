...
object HelloTests extends TestSuite {
  val tests = Tests {
    test("elaborate") {
      /* Parameterization is TBD... */
      (new ChiselStage).run(Seq(ChiselGeneratorAnnotation(()=>LazyModule(configToLazyModule(classOf[WishboneDemoMaster], new BaseSubsystemConfig())).module)))
    }
  }
}
